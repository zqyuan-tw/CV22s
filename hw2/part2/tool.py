...
def get_pseudo_labels(data_loader, model, device, threshold=0.65):
    new_set = []
    with torch.no_grad():
        model.eval()
...
def train(model, train_set, val_set, batch_size, num_epoch, log_path, save_path, device, criterion, scheduler, optimizer, do_semi=False, unlabel_dir=None):
    ...
    if do_semi and unlabel_dir is not None:
        unlabel_set = cifar10_dataset(images=os.listdir(unlabel_dir), transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])]), prefix=unlabel_dir)
        unlabel_loader = DataLoader(unlabel_set, batch_size=batch_size, shuffle=False)
    ...
    for i in range(num_epoch):
        ...
        with open(log_path, 'a') as f :
            ...
            f.write(f'validation loss : {val_loss}  val acc = {val_acc}\n' )
            f.write('============================\n')
    ...
    plot_learning_curve(x, overall_acc, 'Training Accuracy')
    plot_learning_curve(x, overall_loss, 'Training Loss')
    plot_learning_curve(x, overall_val_acc, 'Validation Accuracy')
    plot_learning_curve(x, overall_val_loss, 'Validation_Loss')
